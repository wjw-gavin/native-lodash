...
常用的 JavaScript 工具库，TypeScript 编写。
...
```js
/**
 * @description 延迟 wait 毫秒后调用 fn。调用时，任何附加的参数会传给 func。
 ...
 * @param {Number} radix 生成 uuid 的基数(意味着返回的字符串都是这个基数)，2-二进制，8-八进制，10-十进制，16-十六进制，默认62
 */
...
// 如果判断 format 有值，则再往前的时间显示为 format 格式，否则继续显示几个月前、几年前。
```
 ...
 * @param {String} pos both(两端)|left|right|all，默认 both
 */
